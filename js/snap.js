...
export function snapToRoute(routeCode, lng, lat) {
    ...
    // Tangent window: ±3 points by default. Near endpoints, expand asymmetrically
    // so we get a meaningful tangent rather than a one-sided collapse to the snap point.
    const WINDOW = 3;
    const lastIdx = pts.length - 1;
    let i0 = bestIdx - WINDOW;
    let i1 = bestIdx + WINDOW;
    ...
    return {
        ...
        arcIndex: bestIdx,
        arcMeters: arcLengths[routeCode][bestIdx],
        tangentForward,
        endpointTangent,
    };
}
