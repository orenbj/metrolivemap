...
- **Scheduled feed-reliability audit** — `.github/workflows/feed-reliability.yml` runs `scripts/audit-feeds.js` against the live Metro WS feeds (crons **paused indefinitely** since PR #256, pending re-evaluation — the original target date has passed with no decision yet; manual `workflow_dispatch` still works). This pause is scoped to this workflow's scheduled crons only; `tests.yml` and `deploy-bluehost.yml` run normally. **Source of truth for "does Metro actually populate field X?"** — before wiring up any optional GTFS-RT field (`occupancyStatus`, `arrival.uncertainty`, `tripUpdate.delay`, `position.bearing`, …) consult the latest report's `vehicleFields`/`stuFields` block. Decision rule: ≥30 % nonNull → wire it up; 5–30 % → document partial coverage; <5 % → skip. Local: `node scripts/audit-feeds.js --duration=30m --out=/tmp/r.json`.
