...
- **Declared-stop clamp invariant (PR #210)** — a vehicle marker MUST NEVER visually pass the stop the popup declares as "next". Enforced in `_applySnap` (markers.js): when `snap.arcMeters` projects past the arc of the declared `props.stopId`, snap.arcMeters is pulled back to that arc and `snappedLng/Lat` are re-projected via `lngLatAtArc`. `startDeadReckoning` uses the declared stop's arc as `_drStopArcCap` (with a fallback to the legacy scan-first-ahead when no stopId is available). On warm DR, `_drCurrentArc` is also clamped — handles the case where the feed corrects stopId backward. Helper: `_declaredStopArcCap(props)` returns `{ arc, ascends }` driven by adjacent-arc comparison so the direction check works regardless of `direction_id` (dir=1 trips have descending cache.arcMeters). Counter: `feedStats._markerStats.declaredStopClamp` increments per frame the clamp engages — a baseline near zero between stations with brief spikes around station passes is healthy. **Trade-off accepted explicitly**: when stopId is stale by ≥ 1 stop, the marker yanks backward to the declared stop. This is worse in the rare extreme-staleness case than the pre-fix "drift past stations" behavior, but the user explicitly rejected the drift as "trains flying past stops" (2026-05-20). The clamp matches the popup; honesty trumps smoothness.
