...
- **Tests** — `npm test` runs the Vitest suite (33 files, 799 tests). Run after any change to ETA, snapping, or marker logic. `tests/setup.js` installs an in-memory `localStorage` shim — Node 25+ has a broken built-in `globalThis.localStorage` accessor that collides with jsdom.
