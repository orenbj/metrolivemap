...
- **Non-text contrast for low-luminance route colors** — E (#fdb913), K (#e56db1), J (#adb8bf) fail WCAG 1.4.11 (3:1) on white; the brand palette is preserved. Mitigation: a 1 px inset dark outline scoped to only the failing routes via `.legend-row[data-route="804"|"807"|"910"|"950"] .bar-fill`. Each legend bar also pairs a numeric vehicle count as text (never color-only). `.boarding-badge` gets a 2 px border + inset light outline in dark mode; `.pv2-dot` carries `role="img"` + `aria-label`. If a `data-route` or the KNOWN_FAILING set changes, update both the CSS selector and `tests/route-color-contrast.test.js` (it pins every route hex's contrast).
