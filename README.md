...
- **`gtfs-drift-check.yml`** — Mon 08:00 UTC. Diffs current Metro GTFS against committed `data/trips.json` / `stops.json`; when drift exceeds 5% it files an issue under label `gtfs-drift` AND auto-dispatches `rebuild-gtfs.yml` to self-heal into a rebuild PR, skipping the dispatch if a `gtfs-data`-labeled rebuild PR is already open.
