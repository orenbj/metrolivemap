...
**Lifecycle:** an audit is *Open* while its findings are un-triaged,
*In progress* once they're triaged and some but not all have landed, *Actioned*
...
|---|---|---|
| [`full-app-review-2026-09-02.md`](./full-app-review-2026-09-02.md) | Whole application, ten dimensions: rider UX/mobile, motion, ETA/feed data, station popup, badges/bridges, security/privacy, a11y, performance/resilience, secondary features, CI/pipeline/tests, docs. Every finding independently reproduced or refuted before listing. 63 confirmed (7 high, 38 medium, 18 low), 2 plausible, 21 unverified low/cosmetic. | **In progress** — triaged into seven batches; 1–4 shipped (#629 mobile/a11y blockers, #630 destination + departure + SKIPPED cluster, #631 boot-data integrity + resilience). Remaining: batch 5 (test gaps), batch 6 (motion — every item touches a documented invariant and needs a live capture first), batch 7 (security/docs tail + 17 low/nit + 21 unverified leads). No live-site capture has run yet (see "Honest limits"); `review-live-snapshot.yml` is on `main` and dispatchable. |
