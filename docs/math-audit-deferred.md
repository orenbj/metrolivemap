...
Three previously-deferred items (reverse-direction DR test, `DR_SPEED_FACTOR` regression, `_heavyRailScheduleSpeed` real-world fixture) are RETIRED: the Phase 5b animation pivot (PR #189) deleted the legacy DR integrators and `_heavyRailScheduleSpeed`, so the underlying code being tested no longer exists.
